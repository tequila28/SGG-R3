...
def initialize_embeddings(word_model, is_first_process):
    ...
    for predicate in PSG_REL_CATEGORIES:
        get_doc_cached(predicate)
    for obj in PSG_OBJ_CATEGORIES:
        get_doc_cached(obj)
...
def extract_answer_content(text: str) -> str:
    """
    For the new format with <OBJECT> and <RELATION> tags, combines their content.
    ...
    # 检查是否有 <OBJECT> 和 <RELATION> 标签（新格式）
    ...
    for content, sol, im_id, task_type, box_wh in zip(contents, solution, image_id, task_type_list, box_scale):
        ...
        try:
            ...
            for assign in assignments:
                ...
                gt_id = gt_obj['id']
                gt_box = gt_obj['bbox']
                gt_category = refine_node_edge(gt_id.split('.')[0])
                
                pred_id = pred_obj['id']
                pred_box = pred_obj['bbox']
                pred_category = refine_node_edge(pred_id.split('.')[0])
...
class RelationWeightManager:
    ...
    def _calculate_weight(self, frequency_percent: float) -> float:
        """根据频率计算权重"""
        # 防止除零错误和负频率
        ...
        # 定义对数范围（基于新数据统计）
        min_log = math.log(1.0 / 20.245)  # 最高频率的对数 (on: 20.245%)
        max_log = math.log(1.0 / 0.003)   # 最低频率的对数 (falling off, opening: 0.003%)
    ...
    def _initialize_weights(self) -> Dict[str, float]:
        """初始化谓词权重字典"""
        frequency_data = {
            'on': 20.245, 'beside': 17.210, 'over': 15.031, 'attached to': 7.828,
            'standing on': 7.036, 'in front of': 4.145, 'holding': 3.901, 'in': 3.755,
            'walking on': 2.723, 'parked on': 2.531, 'driving on': 2.144, 'sitting on': 2.021,
            'looking at': 1.948, 'hanging from': 1.475, 'wearing': 1.094, 'carrying': 0.881,
            'riding': 0.762, 'playing': 0.713, 'lying on': 0.561, 'eating': 0.475,
            'running on': 0.436, 'touching': 0.361, 'flying over': 0.321, 'leaning on': 0.307,
            'swinging': 0.278, 'driving': 0.225, 'about to hit': 0.215, 'enclosing': 0.195,
            'talking to': 0.152, 'playing with': 0.130, 'pulling': 0.120, 'painted on': 0.072,
            'crossing': 0.071, 'throwing': 0.068, 'jumping from': 0.062, 'biting': 0.059,
            'slicing': 0.051, 'catching': 0.050, 'on back of': 0.047, 'drinking': 0.044,
            'going down': 0.042, 'guiding': 0.032, 'jumping over': 0.028, 'kicking': 0.028,
            'pushing': 0.026, 'feeding': 0.024, 'entering': 0.018, 'chasing': 0.014,
            'exiting': 0.010, 'cooking': 0.007, 'kissing': 0.006, 'cleaning': 0.006,
            'climbing': 0.004, 'picking': 0.003, 'falling off': 0.003, 'opening': 0.003
        }
        
        return {pred: self._calculate_weight(freq) for pred, freq in frequency_data.items()}
    
    def get_weight(self, predicate: str) -> float:
        """获取谓词权重"""
        return self.weights.get(predicate, self.default_weight)
    
    def get_weights_for_triplets(self, triplets: List[Dict]) -> List[float]:
        """获取三元组列表中所有谓词的权重"""
    ...
    def get_sorted_weights(self, by_frequency: bool = False) -> List[tuple]:
        """按权重排序返回谓词权重列表"""
        if by_frequency:
            # 按频率排序（权重升序）
    ...
    def get_weight_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'total_predicates': len(self.weights)
        }
        ...
        try:
            ...
            if task_type == 'sgg':
                ...
                for gt_rel in gt_rels:
                    sub, obj = refine_node_edge(gt_rel['subject']), refine_node_edge(gt_rel['object'])
...
def is_valid_relationship_structure(relationships):
    ...
    # 检查三个大类的key是否存在
    expected_keys = ["spatial_relations", "static_action_relations", "dynamic_action_relations"]
